...
const TPS = 300; // Transactions per second
const DURATION_SECONDS = 30;
const TOTAL_TRANSACTIONS = TPS * DURATION_SECONDS; // 300 TPS * 30 sec = 9000 txs
const OUTPUT_FILE = 'transaction_hashes_300tps_30sec.txt';

async function main() {
  const provider = new ethers.JsonRpcProvider(RPC_URL);
  const wallet = new ethers.Wallet(PRIVATE_KEY, provider);
  
  console.log('=== STRESS TEST: 300 TPS FOR 30 SECONDS ===');
  ...
  // Clear output file
  fs.writeFileSync(OUTPUT_FILE, `# 300 TPS Stress Test - ${new Date().toISOString()}\n`);
  ...
  // Send all transactions as fast as possible with batching
  const batchSize = 50;
  ...
  for (let batch = 0; batch < TOTAL_TRANSACTIONS; batch += batchSize) {
    ...
    // Progress update every 300 transactions
    if ((batch + batchSize) % 300 === 0 || batch + batchSize >= TOTAL_TRANSACTIONS) {
